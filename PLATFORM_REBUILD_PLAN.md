...
3. **Three things only you can do, none of which I have access to do myself
   this session:**
   - **Push this repo to a GitHub remote.** Nothing has been pushed anywhere
     this whole session — everything so far is local commits on the
     `integration-check` branch. `git remote add origin <your-repo-url>` and
     `git push -u origin integration-check` (then merge to your default
     branch, since scheduled workflows only fire from files on the default
     branch).
