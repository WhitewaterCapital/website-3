...
| Intra/Exitus | `intra-exitus.ts` | **REBUILD (next up)** | Currently RNG entry/exit levels for tickers outside its small real-covered universe (confirmed live: DEMO-A got a real-looking but RNG "(sample)" plan; NVDA correctly abstained with a real reason — "No mean reversion in the window (OU non-reverting)" — so part of this may already be more real than the original audit assumed; needs a closer read of the actual source before rebuilding, not just replacing wholesale). Real OHLCV is available now (Tiingo key is live) — can compute real ATR-based entry zone/stop/targets where the demo path currently fires. |
...
3. **Intra/Exitus rebuild** — up next. Real price data is already available (Tiingo); same real-vs-abstain pattern as Distresse. First step: read `impl/intra-exitus.ts` in full — the live test above showed it already abstains honestly for at least one real ticker (NVDA, "OU non-reverting"), so this may be a partial rebuild + a real UI bug fix (see Roadblocks), not a full RNG replacement.
