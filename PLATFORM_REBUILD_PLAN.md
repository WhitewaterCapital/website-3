...
23. ~~Build Tier C earnings revision-momentum infra (estimate-snapshot tracking)~~ — done 2026-09-14, same day, via a dedicated agent. See dated Log entry below.
24. ~~Extend Cascade Network with NAV-per-share and real typical_volume (Alpaca)~~ — done 2026-09-14, same day, via a dedicated agent. See dated Log entry below.
25. ~~Research + build a self-tuning (adaptive) Kalman filter for pairs trading, "in the spirit of" Susquehanna's publicly-stated philosophy~~ — done 2026-09-14, same day. See dated Log entry below.

## Log (continued)

- **2026-09-14 (same day, continued) — commit/push confirmation for items 21-22.** Philip cleared a second occurrence of the stuck `.git/index.lock` himself (his own retry screenshot showed `File exists` again, then "cannot pull with rebase: you have unstaged changes" — meaning his first `add`/`commit` never completed because of the lock) and ran the standard sequence a second time. This session cleared the lock as a courtesy via `mv .git/index.lock ".git/index.lock.stale-<ts>"` (rename, not delete — the only safe non-git operation available given the `unlink()` restriction) and confirmed no in-progress rebase/merge state was blocking him (`ls .git/ | grep -i rebase\|merge` — only an unrelated, non-blocking `AUTO_MERGE` artifact present). His next attempt succeeded cleanly: commit `21fb08f` → rebased onto the meanwhile-arrived `chore(clock)` commit `10ff098` → pushed as `cea103d`. Confirmed directly against `git log --oneline` this pass: `cea103d` ("Add WW-Chaos live feed adapter and Tier B earnings SUE signals") sits on top of `10ff098` on `integration-check`, exactly as expected. Also mid-turn this session, Philip gave a new standing instruction for the rest of the engagement: run everything through to completion autonomously and present one final consolidated commit/push instruction at the end, rather than pausing after every wave.

  Philip then asked to keep going, and separately asked for a self-tuning Kalman filter for equities pairs ("look at what susquehanna builds and do smth similar or as good as possible"). Dispatched two more agents in parallel (Wave 3), each briefed with the standing no-git rule, full file-ownership boundaries, and the "read everything, verify for real" discipline:

  **(23) Tier C earnings revision-momentum.** The prior wave's SUE build (item 22) left `sue` correctly `null` for every event by construction (pre-print, no real EPS yet), but left "estimate revision momentum" — whether the Street's consensus has been rising or falling into the print, a real and separate signal from SUE — unbuilt, since Alpha Vantage's free endpoint only exposes the CURRENT consensus, not point-in-time history. This agent built the missing piece: `earnings-engine/ee/revisions.py`, an append-only local snapshot log (`state/estimate_snapshots.jsonl`) that records each day's `eps_estimate` per ticker/event the first time it's seen that day, then `compute_revision(current_estimate, prior, history_count)` diffs the current pull against the most recent PRIOR snapshot to return `(direction, pct, reason)` — `"raised"` / `"lowered"` / `"unchanged"` / an honest abstain reason (`"no-prior-snapshot"` on day one, `"insufficient-history"`, etc.) — never a fabricated trend from a single data point. Wired into `export.py` (schema bumped to 1.2.0, engine 0.3.0) via a new `record_and_compute_revisions()` orchestrator, with `EE_EXPORT_AS_OF_DATE` added as an environment override specifically so this mechanism could be exercised deterministically across two "different days" without waiting on the calendar. Independently re-ran this exact demonstration this pass — ran the export once, then again a second time under a different `EE_EXPORT_AS_OF_DATE` with a lowered `eps_estimate` for AAPL — and confirmed the second run reports `revision_direction: "lowered"`, `revision_pct` matching the actual (3.08-3.99)/3.99 arithmetic exactly. `src/lib/models/earnings-export.ts` gained the matching `revision_direction`/`revision_pct`/`revision_abstain_reason` fields and a new `EarningsRevisionDirection` type; `earnings-move.ts` surfaces this as a fourth honest context line (`revisionNoteFor()`) alongside insider positioning, factor momentum, and SUE — still never blended into `lean`/`score`, same insider-only design as every prior wave. Test suite grew 15 → 35 (20 new in `test_revisions.py`); re-ran `python3 -m unittest discover -s tests` inside `earnings-engine/` directly this pass and confirmed **35/35 pass**.

  **(24) Cascade Network NAV + typical_volume.** The prior wave's real iShares holdings adapter (item 19) still needed two more real inputs before Cascade Network's pressure calculation would mean anything beyond raw share-count deltas: NAV-per-share (to price flow in dollars, not just shares) and `typical_volume` per constituent (to normalize a flow against how liquid the name actually is). Built `cde/adapters/ishares_nav.py` — a regex-based parser against the same iShares fund-overview pages already confirmed reachable, extracting the NAV phrase directly from page text (documented in its own module docstring as a genuine disagreement between two separate WebFetch attempts about the page's exact structure, resolved by matching on the stable phrase rather than a brittle DOM path) — gated on the same `CASCADE_LIVE_HOLDINGS` flag as the holdings adapter. Built `cde/adapters/alpaca_volume.py` — a sealed, deliberately-duplicated copy of the Alpaca auth/fetch pattern already used in `chaos-engine` and (as of this pass) `kalman-engine`, pulling `1Day` bars and computing trailing-21-day dollar volume, gated on `ALPACA_API_KEY_ID`/`ALPACA_API_SECRET_KEY` — with its own `typical_volume_provenance` field (`"live-alpaca"` / `"not-configured"` / `"alpaca-fetch-failed"` / `"synthetic-demo"`) so a failed live fetch is stated as failed, never silently swapped for synthetic numbers under a live label. Test suite grew 23 → 42 (19 new: `test_ishares_nav.py`, `test_alpaca_volume.py`, two new integration cases in `test_export.py`); re-ran `python3 -m unittest discover -s tests` inside `cascade-data-engine/` directly this pass and confirmed **42/42 pass**. Both real vendor endpoints (iShares NAV page, Alpaca) remain unreachable from every sandbox available this session (same confirmed network wall) — exports remain honestly `data_provenance: "synthetic-demo"` until run from an environment with real egress.

  Integration verification for both (23, 24), done independently rather than trusted from either agent's report: re-ran both real test suites directly (35/35, 42/42, matching each agent's own report exactly); `npx tsc --noEmit` on the whole repo — same 2 pre-existing `TickerHubClient.tsx` errors, nothing new; `npx eslint` on every touched TS file — clean.

- **2026-09-14 (same day, continued) — Kalman Pairs (item 25), Philip's request: "build a kalman filter for equities which learns from itself... look at what susquehanna builds."** Researched Susquehanna (SIG) honestly first: SIG is a private firm with no public documentation of any proprietary trading system, so nothing here claims to replicate an actual SIG model. Its only publicly confirmed characteristic (via Grokipedia) is a general philosophy of probabilistic/Bayesian decision-making with roots in poker and continuous real-time iteration — this build is framed throughout as "in that general spirit," never as "a SIG system," including an explicit disclaimer on the `/kalman` page itself.

  Built `kalman-engine/` (sealed, same layout convention as every other engine): `kf/cointegration.py` — a from-scratch Engle & Granger (1987) two-step test (OLS hedge ratio + a hand-rolled augmented Dickey-Fuller regression on the residuals, since `statsmodels` isn't importable in any sandbox available this session), using MacKinnon's (2010) asymptotic N=2 critical values as a documented, conservatively-biased approximation (biased toward correctly abstaining, not toward false positives) rather than claiming exact finite-sample values it can't compute without `statsmodels`. `kf/filter.py` — the adaptive Kalman filter core: standard state-space pairs-trading formulation (state `[mu_t, gamma_t]`, observation `y1_t = mu_t + gamma_t * y2_t + eps_t`, random-walk transition — Vidyamurthy 2004, Chan 2013 ch.5), run on LOG prices rather than raw prices (a stated, reasoned deviation: keeps R comparable in scale across differently-priced pairs), with the actual "learns from itself" mechanism being online re-estimation of Q and R from the filter's own residuals every step — the posterior-residual form for R and the innovation/Kalman-gain form for Q, both from Akhlaghi, Zhou & Huang (2017, arXiv:1702.00884), themselves grounded in Mehra's (1970) original innovation-based adaptive-filtering framework — with floors on both so neither collapses to zero and the filter can't become overconfident. Explicitly named what this does NOT implement: EM/Kalman-smoother-based batch covariance learning (Shumway & Stoffer 1982) is a real, different, legitimate alternative method from the same literature that was considered and not built this pass, since it's a batch/offline technique rather than the online "learns as it runs" mechanism the task asked for.

  I independently verified this build myself before logging it as done, reading the actual code rather than trusting the agent's report: read `kf/filter.py`'s full docstring and the update-equation code beneath it line by line — confirmed the code (`R = beta*R + (1-beta)*(e**2 + H@P_new@H.T)`, `Q = beta*Q + (1-beta)*outer(K*v, K*v)`, diagonal-only Q floor, exact-symmetry enforcement on P after each update) matches the cited Akhlaghi et al. equations exactly, not just the prose description of them. Read the opening of `kf/cointegration.py` and confirmed its stated ADF critical-value approximation and bias direction are accurately described. Read the abstention/provenance logic in `kf/export.py` and confirmed it follows the same honest-abstention, no-mixed-provenance pattern as every other engine. Independently re-ran `python3 -m unittest discover -s tests -v` inside `kalman-engine/` myself — **21/21 pass**, including tests that specifically verify the adaptive mechanism is real and not cosmetic (`test_r_responds_to_a_genuine_regime_change_within_one_run`, `test_q_and_r_never_collapse_to_the_floor`, `test_no_lookahead_state_at_t_uses_only_data_up_to_t`) alongside the cointegration-recovery and honest-abstention tests. Ran `npx tsc --noEmit` on the whole repo — same 2 pre-existing, unrelated `TickerHubClient.tsx` errors, nothing new. Ran `npx eslint` on all four new/changed TS files (`src/lib/models/kalman-export.ts`, `src/lib/kalman.ts`, `src/lib/models/impl/kalman-pairs.ts`, `src/app/kalman/page.tsx`) — one flag on `src/app/kalman/page.tsx` (`react/jsx-no-comment-textnodes` on the `// Kalman Pairs` section label), confirmed NOT specific to this file by running the same lint rule against `dashboard/page.tsx` and `hub/page.tsx`: both fail identically on their own pre-existing `// The Desk` / `// Ticker Hub` labels, so this is a pre-existing site-wide pattern, not something this build introduced — left as-is, consistent with this session's policy of not touching unrelated pre-existing issues.

  Wired `kalmanPairs` into the registry myself (not delegated — this is core shared infrastructure, kept off-limits to every subagent all session): imported into `src/lib/models/registry.ts`, added to the `models` object, `equityModels` array, and `MODELS` array. Added the matching "Open Kalman Pairs →" conditional link in `src/app/models/page.tsx` (same per-`m.id` pattern as every other live model) and a `/kalman` entry in `src/app/dashboard/page.tsx`'s `MODULE_LINKS`. Re-ran `tsc --noEmit` and `eslint` across the whole repo after wiring — clean (same pre-existing findings only, nothing new).

  Uses Alpaca (daily bars, universe AAPL/MSFT/NVDA/JPM/XOM/KO, 15 candidate pairs) via a sealed, deliberately-duplicated adapter (`kf/adapters/alpaca_bars.py`, same auth pattern as `chaos-engine`/`cascade-data-engine`) — same unexercised-with-a-real-key caveat as every other Alpaca integration this session; no real key has been available in any sandbox this session to test the live path end to end. `/kalman` page shows every pair's cointegration status, current hedge ratio, and a Q/R init→now table so a member can see the "learns from itself" claim directly rather than take it on faith.

  **Still not committed** — on top of everything above: new `earnings-engine/ee/revisions.py`, `earnings-engine/tests/test_revisions.py`, `earnings-engine/state/`; changed `earnings-engine/ee/config.py`, `ee/export.py`, `ee/synthetic.py`, `README.md`, `src/lib/models/earnings-export.ts`, `src/lib/models/impl/earnings-move.ts`; new `cascade-data-engine/cde/adapters/ishares_nav.py`, `cde/adapters/alpaca_volume.py`, `cascade-data-engine/tests/test_ishares_nav.py`, `tests/test_alpaca_volume.py`; changed `cascade-data-engine/cde/export.py`, `README.md`; new `kalman-engine/` (entire package), `public/data/kalman/`, `src/app/kalman/`, `src/lib/kalman.ts`, `src/lib/models/impl/kalman-pairs.ts`, `src/lib/models/kalman-export.ts`; changed `src/lib/models/registry.ts`, `src/app/models/page.tsx`, `src/app/dashboard/page.tsx`. Per the standing instruction, this session will run through the rest of the genuinely buildable backlog before presenting one final, single commit/push instruction for Philip's own Terminal.
